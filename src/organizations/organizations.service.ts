...
export class OrganizationsService {
  ...
  async create(createOrganizationDto: CreateOrganizationDto) {
    await this.checkIfOrganizationsExists(createOrganizationDto.name);
  ...
  async update(id: number, updateOrganizationDto: UpdateOrganizationDto) {
    ...
    await this.checkIfOrganizationsExists(updateOrganizationDto.name, id);
  ...
  private async checkIfOrganizationsExists(name: string, id?: number) {
    const doesOrganizationsExists = await this.prismaService.organization.findFirst({
      where: { name},
    });
    if (doesOrganizationsExists) {
      if (id && doesOrganizationsExists.id !== id) {
        //this is update case
